...
public class Utils
{
    ...
    static {
        ...
        Utils.no_faction = Helpful.translate("&cYou are not in a faction.");
        Utils.role = Helpful.translate("&cYour cannot do this with this role.");
        Utils.faction_not_found = Helpful.translate("&cFaction not found.");
        Utils.no_ally = Helpful.translate("&cAllies are currently disabled this map.");
        Utils.no_permission = Helpful.translate("&cNo permission.");
        Utils.no_console = Helpful.translate("&cNo console.");
        Utils.target_offline = Helpful.translate("&cThat player is currently offline.");
        Utils.STICK = "\u2503";
    ...
    public static boolean ensure(final UUID uuid) {
        if (String.valueOf(name(uuid)).equals("null")) {
            AntiPhase.getInstance().getLogger().warning(uuid + " didn't have a cached name.");
            return false;
        }
        return true;
    }
