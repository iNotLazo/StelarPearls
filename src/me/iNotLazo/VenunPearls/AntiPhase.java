...
import me.iNotLazo.VenunPearls.Helper.*;
import me.iNotLazo.VenunPearls.Helper.Timer;
...
public class AntiPhase extends JavaPlugin implements Listener
{
    ...
    public ConcurrentMap<Object, Object> pearlMap;
    private static AntiPhase instance;
    private ConfigFile configfile;
    private Timer timer;
    public ArrayList<UUID> onebyone;
    
    public void onEnable() {
        AntiPhase.instance = this;
        this.playersToLastTeleport = new ArrayList<Player>();
        this.saveDefaultConfig();
        this.getLogger().info("Configuration file has been updated.");
        (this.timer = new Timer(this)).enable();
        Bukkit.getPluginManager().registerEvents((Listener)this, (Plugin)this);
        this.getLogger().info(String.valueOf(String.valueOf(String.valueOf(this.getName()))) + " has been enabled!");
        this.configfile = new ConfigFile();
        this.onebyone = new ArrayList<UUID>();
        this.getCommand("spearls").setExecutor((CommandExecutor)new CMD());
    }
    
    public void onDisable() {
        AntiPhase.instance = null;
        this.getLogger().info(String.valueOf(String.valueOf(String.valueOf(this.getName()))) + " has been disabled!");
    }
    ...
    public void onDamage(final EntityDamageByEntityEvent e) {
    	if (e.getDamager() instanceof EnderPearl) {
	    	...
	    	//final EnderPearl enderPearl = (EnderPearl) e.getEntity();
	    	EnderPearl pearl = (EnderPearl) e.getDamager();
	    	Player player = (Player) pearl.getShooter();
	        Player damaged = (Player) e.getEntity();
	        if (e.getDamager().getType().equals(EntityType.ENDER_PEARL) && player != null && damaged != null) {
	        	//Location loc = new Location(damaged.getWorld(), damaged.getLocation().getX(), damaged.getLocation().getY(), damaged.getLocation().getZ());
	        	Location loc = getOneByOne(pearl.getLocation(), player);
	        	loc.setPitch(player.getLocation().getPitch());
	        	loc.setYaw(player.getLocation().getYaw());
	        	//player.teleport(loc, PlayerTeleportEvent.TeleportCause.ENDER_PEARL);
	        	
	        	pearl.setMetadata("onebyone", (MetadataValue)new FixedMetadataValue((Plugin)this, (Object)true));
	        	//player.teleport(this.getOneByOne(damaged.getLocation(), player), PlayerTeleportEvent.TeleportCause.ENDER_PEARL);
	        	
	        	//onebyone.add(player.getUniqueId());
	        }
    	} else {
    		return;
    	}
    ...
    public void onPearlLaunch(final ProjectileLaunchEvent event) {
        ...
        if (event.getEntity() instanceof EnderPearl) {
		    ...
		    enderPearl.setMetadata("tali", (MetadataValue)new FixedMetadataValue((Plugin)this, (Object)true));
		    }*/
		    if (((block.getRelative(BlockFace.UP).getType() != Material.ACACIA_FENCE_GATE 
		    		...
		    		&& (block.getRelative(BlockFace.DOWN).getType() != Material.ACACIA_FENCE_GATE
		    				|| block.getRelative(BlockFace.DOWN).getType() != Material.BIRCH_FENCE_GATE
		    				|| block.getRelative(BlockFace.DOWN).getType() != Material.DARK_OAK_FENCE_GATE
		    				|| block.getRelative(BlockFace.DOWN).getType() != Material.FENCE_GATE
		    				|| block.getRelative(BlockFace.DOWN).getType() != Material.JUNGLE_FENCE_GATE
		    				|| block.getRelative(BlockFace.DOWN).getType() != Material.SPRUCE_FENCE_GATE)
		    		&& block.getRelative(BlockFace.DOWN).getType().isSolid()
		    		
		    		&& !block.getLocation().getBlock().getRelative(BlockFace.valueOf(this.getDirectionCompleteName(player))).getType().isSolid())) {
		        enderPearl.setMetadata("selfair", (MetadataValue)new FixedMetadataValue((Plugin)this, (Object)true));
		    }
            /*if (block.getRelative(BlockFace.UP).getType() == Material.FENCE_GATE 
            ...
            if (block != null 
            		&& getInstance().getConfig().getBoolean("cPearl") 
            		&& this.isCobbleWall(block) 
            		&& block.getRelative(BlockFace.UP).getType().isSolid()
            		&& !block3.getLocation().getBlock().getRelative(BlockFace.SELF).getType().isSolid()) {
                enderPearl.setMetadata("cpearl", (MetadataValue)new FixedMetadataValue((Plugin)this, (Object)true));
            }
            ...
            if (block.getRelative(BlockFace.SELF).getType() == Material.FENCE_GATE 
            	...
            	if (this.getConfig().getBoolean("Invalid-Pearl-Command")) {
                    Bukkit.getServer().dispatchCommand((CommandSender)Bukkit.getConsoleSender(), this.getConfig().getString("Command").replace("%player%", player.getName()));
                }
                HCFactions.getInstance().getEnderpearlHandler().quit(player);
                if (this.getConfig().getBoolean("Invalid-Pearl-Message")) {
                    player.sendMessage(Helpful.translate(this.getConfig().getString("Message")));
                }
                new BukkitRunnable() {
                    public void run() {
                        if (AntiPhase.getInstance().getConfig().getBoolean("Invalid-Pearl-Remove-Timer")) {
                            AntiPhase.getInstance().getTimer().cooldown.remove(player.getUniqueId());
                        }
                    }
                }.runTaskLater((Plugin)this, 2L);
                if (this.getConfig().getBoolean("return-pearl")) {
                    player.getInventory().addItem(new ItemStack[] { new ItemStack(Material.ENDER_PEARL, 1) });
                }
                event.setCancelled(true);
            }*/
            if (block != null 
            		&& getInstance().getConfig().getBoolean("Chest-Pearl")) {
                ...
                /*if (this.isChest(block) && block.getRelative(BlockFace.UP).getType().isSolid() && block.getLocation().getBlock().getRelative(BlockFace.valueOf(this.getDirectionCompleteName(player))).getType().isSolid()) {
                	if (this.getConfig().getBoolean("Invalid-Pearl-Command")) {
                        Bukkit.getServer().dispatchCommand((CommandSender)Bukkit.getConsoleSender(), this.getConfig().getString("Command").replace("%player%", player.getName()));
                    }
                    HCFactions.getInstance().getEnderpearlHandler().quit(player);
            ...
            if (block != null && getInstance().getConfig().getBoolean("EndPortal-Pearl")
            		&& this.isEndPortal(block) 
            		&& block.getRelative(BlockFace.UP).getType().isSolid()
            		&& !block3.getLocation().getBlock().getRelative(BlockFace.SELF).getType().isSolid()) {
                enderPearl.setMetadata("cpearl", (MetadataValue)new FixedMetadataValue((Plugin)this, (Object)true));
            }
            if (block != null && getInstance().getConfig().getBoolean("Piston-Pearl") 
            		&& this.isPiston(block) 
            		&& block.getRelative(BlockFace.UP).getType().isSolid()
            		&& !block3.getLocation().getBlock().getRelative(BlockFace.SELF).getType().isSolid()) {
                enderPearl.setMetadata("cpearl", (MetadataValue)new FixedMetadataValue((Plugin)this, (Object)true));
            }
            if (block != null && getInstance().getConfig().getBoolean("Bed-Pearl") 
            		&& this.isBed(block) 
            		&& block.getRelative(BlockFace.UP).getType().isSolid()
            		&& !block3.getLocation().getBlock().getRelative(BlockFace.SELF).getType().isSolid()) {
                enderPearl.setMetadata("cpearl", (MetadataValue)new FixedMetadataValue((Plugin)this, (Object)true));
            ...
            if (block != null 
            		&& !block3.getLocation().getBlock().getRelative(BlockFace.SELF).getType().isSolid() 
            		&& block.getRelative(BlockFace.UP).getType().isSolid()
            		&& ((getInstance().getConfig().getBoolean("TalibanPearl.Slabs") 
            		&& this.isSlab(block)) 
            		|| 
            		(getInstance().getConfig().getBoolean("TalibanPearl.Stairs") 
            		&& this.isStair(block)))) {
                enderPearl.setMetadata("tali", (MetadataValue)new FixedMetadataValue((Plugin)this, (Object)true));
            }
            /*if (block != null && block2.getLocation().getBlock().getRelative(BlockFace.valueOf(this.getDirectionCompleteName(player))).getType().isSolid() 
            		&& getInstance().getConfig().getBoolean("TalibanPearl.Block-if-block-in-front")
            		&& ((getInstance().getConfig().getBoolean("TalibanPearl.Slabs") && this.isSlab(block)) 
            		|| (getInstance().getConfig().getBoolean("TalibanPearl.Stairs") && this.isStair(block)))) {
            	if (this.getConfig().getBoolean("Invalid-Pearl-Command")) {
                    Bukkit.getServer().dispatchCommand((CommandSender)Bukkit.getConsoleSender(), this.getConfig().getString("Command").replace("%player%", player.getName()));
                }
                HCFactions.getInstance().getEnderpearlHandler().quit(player);
            ...
            /*if (block != null && getInstance().getConfig().getBoolean("TalibanPearl.Stairs") 
            		&& this.isStair(block) && ((((Stairs)block.getRelative(BlockFace.SELF).getState().getData()).getFacing() == BlockFace.WEST 
            		&& BlockFace.valueOf(this.getDirectionCompleteName(player)) == BlockFace.EAST) || (((Stairs)block.getRelative(BlockFace.SELF).getState().getData()).getFacing() == BlockFace.valueOf(this.getDirectionCompleteName(player)) 
            		&& BlockFace.valueOf(this.getDirectionCompleteName(player)) == ((Stairs)block.getRelative(BlockFace.SELF).getState().getData()).getFacing()) || (((Stairs)block.getRelative(BlockFace.SELF).getState().getData()).getFacing() == BlockFace.EAST 
            		&& BlockFace.valueOf(this.getDirectionCompleteName(player)) == BlockFace.WEST) || (((Stairs)block.getRelative(BlockFace.SELF).getState().getData()).getFacing() == BlockFace.NORTH 
            		&& BlockFace.valueOf(this.getDirectionCompleteName(player)) == BlockFace.SOUTH) || (((Stairs)block.getRelative(BlockFace.SELF).getState().getData()).getFacing() == BlockFace.SOUTH && BlockFace.valueOf(this.getDirectionCompleteName(player)) == BlockFace.NORTH))) {
            	if (this.getConfig().getBoolean("Invalid-Pearl-Command")) {
                    Bukkit.getServer().dispatchCommand((CommandSender)Bukkit.getConsoleSender(), this.getConfig().getString("Command").replace("%player%", player.getName()));
                }
                HCFactions.getInstance().getEnderpearlHandler().quit(player);
        ...
        if (block != null && (block.getRelative(BlockFace.UP).getType() == Material.ACACIA_FENCE_GATE
        		|| block.getRelative(BlockFace.UP).getType() == Material.BIRCH_FENCE_GATE
        		|| block.getRelative(BlockFace.UP).getType() == Material.DARK_OAK_FENCE_GATE
        		|| block.getRelative(BlockFace.UP).getType() == Material.FENCE_GATE
        		|| block.getRelative(BlockFace.UP).getType() == Material.JUNGLE_FENCE_GATE
        		...
        		&& !((Gate)block.getRelative(BlockFace.UP).getState().getData()).isOpen() 
        		&& block.getRelative(BlockFace.SELF).getType() == Material.FENCE_GATE 
        		&& (block.getRelative(BlockFace.SELF).getType() == Material.ACACIA_FENCE_GATE
                		|| block.getRelative(BlockFace.SELF).getType() == Material.BIRCH_FENCE_GATE
                		|| block.getRelative(BlockFace.SELF).getType() == Material.DARK_OAK_FENCE_GATE
                		|| block.getRelative(BlockFace.SELF).getType() == Material.FENCE_GATE
                		|| block.getRelative(BlockFace.SELF).getType() == Material.JUNGLE_FENCE_GATE
                		|| block.getRelative(BlockFace.SELF).getType() == Material.SPRUCE_FENCE_GATE)
        		&& !((Gate)block.getRelative(BlockFace.SELF).getState().getData()).isOpen() ) {
        	if (this.getConfig().getBoolean("Invalid-Pearl-Command")) {
                Bukkit.getServer().dispatchCommand((CommandSender)Bukkit.getConsoleSender(), this.getConfig().getString("Command").replace("%player%", player.getName()));
            }
        ...
        if (block != null) {
            ...
            if (player.getLocation().getBlock().getRelative(BlockFace.DOWN).getType() != Material.AIR  
            		&& getInstance().getConfig().getBoolean("invalid-if-block-in-front")
    ...
    private boolean isSlab(final Block block) {
        final Material material = block.getType();
        return material != null && material != Material.AIR && (material == Material.WOOD_STEP || material == Material.STEP);
    }
    ...
    public void onPearlLand(final ProjectileHitEvent event) {
        ...
        final Player player = (Player)enderPearl.getShooter();
        if (enderPearl.hasMetadata("onebyone") && this.getOneByOne(enderLocation, player) != null) {
            if (this.getOneByOne(enderLocation, player) != null) {
    ...
    public void onClip(final PlayerTeleportEvent e) {
        ...
        if (e.getCause() != null && e.getCause() == PlayerTeleportEvent.TeleportCause.ENDER_PEARL && p.getItemInHand().getType() == Material.ENDER_PEARL) {
        	/*if (to.getBlock().getRelative(BlockFace.UP).getType().isSolid() 
            		...
            		&& to.getBlock().getType() != Material.ACACIA_FENCE
            		&& to.getBlock().getType() != Material.BIRCH_FENCE
            		&& to.getBlock().getType() != Material.DARK_OAK_FENCE
            		&& to.getBlock().getType() != Material.FENCE_GATE
            		&& to.getBlock().getType() != Material.FENCE
            		&& to.getBlock().getType() != Material.NETHER_FENCE
            		&& to.getBlock().getType() != Material.JUNGLE_FENCE
            		&& to.getBlock().getType() != Material.SPRUCE_FENCE
            		&& to.getBlock().getType() != Material.ANVIL
            		&& to.getBlock().getType() != Material.TRAP_DOOR) {
            	if (this.getConfig().getBoolean("Invalid-Pearl-Command")) {
                    Bukkit.getServer().dispatchCommand((CommandSender)Bukkit.getConsoleSender(), this.getConfig().getString("Command").replace("%player%", p.getName()));
                }
                HCFactions.getInstance().getEnderpearlHandler().quit(p);
                if (this.getConfig().getBoolean("Invalid-Pearl-Message")) {
                    p.sendMessage(Helpful.translate(this.getConfig().getString("Message")));
                }
                new BukkitRunnable() {
                    public void run() {
                        if (AntiPhase.getInstance().getConfig().getBoolean("Invalid-Pearl-Remove-Timer")) {
                            AntiPhase.getInstance().getTimer().cooldown.remove(p.getUniqueId());
                        }
                    }
                }.runTaskLater((Plugin)this, 2L);
                if (this.getConfig().getBoolean("return-pearl")) {
            ...
            if (to.getBlock().getRelative(BlockFace.UP).getType().isSolid() 
            		&& to.getBlock().getRelative(BlockFace.DOWN).getType().isSolid() 
            		&& block2.getRelative(BlockFace.DOWN).getType().isSolid() 
            		&& !this.isStair(to.getBlock()) && !this.isSlab(to.getBlock()) 
            		&& !this.isCobbleWall(to.getBlock()) 
            		&& !this.isEndPortal(to.getBlock()) && !this.isPiston(to.getBlock()) && !this.isBed(to.getBlock()) 
            		&& !this.isChest(to.getBlock())
            		&& !this.isSign(to.getBlock())
            		&& !this.isAnvil(to.getBlock())
            		&& !this.isEnchantmentTable(to.getBlock()) 
            		&& (to.getBlock().getType() != Material.ACACIA_FENCE_GATE
            		...
            		&& to.getBlock().getType() != Material.TRAP_DOOR) {
                if (this.getConfig().getBoolean("Invalid-Pearl-Command")) {
                    Bukkit.getServer().dispatchCommand((CommandSender)Bukkit.getConsoleSender(), this.getConfig().getString("Command").replace("%player%", e.getPlayer().getName()));
                }
                if (this.getConfig().getBoolean("Invalid-Pearl-Message")) {
                    p.sendMessage(Helpful.translate(this.getConfig().getString("Message")));
                }
            ...
            if ((to.getBlock().getRelative(BlockFace.UP).getType() == Material.ACACIA_FENCE_GATE
	            		|| to.getBlock().getRelative(BlockFace.UP).getType() == Material.BIRCH_FENCE_GATE
	            		|| to.getBlock().getRelative(BlockFace.UP).getType() == Material.DARK_OAK_FENCE_GATE
	            		|| to.getBlock().getRelative(BlockFace.UP).getType() == Material.FENCE_GATE
	            		|| to.getBlock().getRelative(BlockFace.UP).getType() == Material.JUNGLE_FENCE_GATE
	            		|| to.getBlock().getRelative(BlockFace.UP).getType() == Material.SPRUCE_FENCE_GATE)
            		&& (to.getBlock().getRelative(BlockFace.SELF).getType() == Material.ACACIA_FENCE_GATE
            				|| to.getBlock().getRelative(BlockFace.SELF).getType() == Material.BIRCH_FENCE_GATE
            				|| to.getBlock().getRelative(BlockFace.SELF).getType() == Material.DARK_OAK_FENCE_GATE
            				|| to.getBlock().getRelative(BlockFace.SELF).getType() == Material.FENCE_GATE
            				|| to.getBlock().getRelative(BlockFace.SELF).getType() == Material.JUNGLE_FENCE_GATE
            				|| to.getBlock().getRelative(BlockFace.SELF).getType() == Material.SPRUCE_FENCE_GATE)
            ...
            if ((to.getBlock().getRelative(BlockFace.UP).getType() != Material.ACACIA_FENCE_GATE
            		...
            		&& (to.getBlock().getRelative(BlockFace.SELF).getType() == Material.ACACIA_FENCE_GATE
            				...
            				|| to.getBlock().getRelative(BlockFace.SELF).getType() == Material.SPRUCE_FENCE_GATE)
            		&& !((Gate)to.getBlock().getRelative(BlockFace.SELF).getState().getData()).isOpen()
            		&& (to.getBlock().getRelative(BlockFace.DOWN).getType() == Material.ACACIA_FENCE_GATE
            				|| to.getBlock().getRelative(BlockFace.DOWN).getType() == Material.BIRCH_FENCE_GATE
            				|| to.getBlock().getRelative(BlockFace.DOWN).getType() == Material.DARK_OAK_FENCE_GATE
            				|| to.getBlock().getRelative(BlockFace.DOWN).getType() == Material.FENCE_GATE
            				|| to.getBlock().getRelative(BlockFace.DOWN).getType() == Material.JUNGLE_FENCE_GATE
            				|| to.getBlock().getRelative(BlockFace.DOWN).getType() == Material.SPRUCE_FENCE_GATE)
            ...
            if (!getInstance().getConfig().getBoolean("Chest-Pearl") 
            		&& (to.getBlock().getRelative(BlockFace.SELF).getType() == Material.CHEST 
            		|| to.getBlock().getRelative(BlockFace.SELF).getType() == Material.TRAPPED_CHEST) 
            		&& to.getBlock().getRelative(BlockFace.UP).getType().isSolid()
            		&& (to.getBlock().getRelative(BlockFace.UP).getType() != Material.ACACIA_FENCE_GATE
            				|| to.getBlock().getRelative(BlockFace.UP).getType() != Material.BIRCH_FENCE_GATE
            				|| to.getBlock().getRelative(BlockFace.UP).getType() != Material.DARK_OAK_FENCE_GATE
            				|| to.getBlock().getRelative(BlockFace.UP).getType() != Material.FENCE_GATE
            				|| to.getBlock().getRelative(BlockFace.UP).getType() != Material.JUNGLE_FENCE_GATE
            ...
            if (!getInstance().getConfig().getBoolean("Piston-Pearl") && this.isPiston(to.getBlock().getRelative(BlockFace.SELF)) && to.getBlock().getRelative(BlockFace.UP).getType().isSolid()) {
            	if (this.getConfig().getBoolean("Invalid-Pearl-Command")) {
                    Bukkit.getServer().dispatchCommand((CommandSender)Bukkit.getConsoleSender(), this.getConfig().getString("Command").replace("%player%", p.getName()));
                }
                HCFactions.getInstance().getEnderpearlHandler().quit(p);
            ...
            if (!getInstance().getConfig().getBoolean("EndPortal-Pearl") && to.getBlock().getRelative(BlockFace.SELF).getType() == Material.ENDER_PORTAL_FRAME && to.getBlock().getRelative(BlockFace.UP).getType().isSolid()) {
            	if (this.getConfig().getBoolean("Invalid-Pearl-Command")) {
                    Bukkit.getServer().dispatchCommand((CommandSender)Bukkit.getConsoleSender(), this.getConfig().getString("Command").replace("%player%", p.getName()));
                }
                HCFactions.getInstance().getEnderpearlHandler().quit(p);
            ...
            if (!getInstance().getConfig().getBoolean("Bed-Pearl") && (to.getBlock().getRelative(BlockFace.SELF).getType() == Material.BED_BLOCK || to.getBlock().getRelative(BlockFace.SELF).getType() == Material.BED) && to.getBlock().getRelative(BlockFace.UP).getType().isSolid()) {
            	if (this.getConfig().getBoolean("Invalid-Pearl-Command")) {
                    Bukkit.getServer().dispatchCommand((CommandSender)Bukkit.getConsoleSender(), this.getConfig().getString("Command").replace("%player%", p.getName()));
                }
                HCFactions.getInstance().getEnderpearlHandler().quit(p);
            ...
            if (!getInstance().getConfig().getBoolean("cPearl") && to.getBlock().getRelative(BlockFace.SELF).getType() == Material.COBBLE_WALL) {
            	if (this.getConfig().getBoolean("Invalid-Pearl-Command")) {
                    Bukkit.getServer().dispatchCommand((CommandSender)Bukkit.getConsoleSender(), this.getConfig().getString("Command").replace("%player%", p.getName()));
                }
                HCFactions.getInstance().getEnderpearlHandler().quit(p);
    ...
    public static AntiPhase getInstance() {
        return AntiPhase.instance;
    }
